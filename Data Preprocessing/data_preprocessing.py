...
def data_transformation(df):
    ...
    # Apply log transformation to salary and age (if values > 0)
    df = df.withColumn("log_salary", when(col("salary") > 0, log(col("salary"))).otherwise(None))
    df = df.withColumn("log_age", when(col("age") > 0, log(col("age"))).otherwise(None))

    print("DataFrame after log transformation:")
    df.select("id", "salary", "log_salary", "age", "log_age").show(truncate=False)
...
if __name__ == "__main__":
    ...
    df_transformed = data_transformation(cleaned)
    data_reduction(cleaned)
